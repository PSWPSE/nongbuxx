...
class NewsConverter:
    ...
    def clean_response(self, response):
        ...
        # 줄바꿈 처리 개선 - 모든 포맷팅 요소에 적절한 줄바꿈 추가
        text = text.replace('\\n', '\n')
        
        # (출처: ...) 패턴을 찾아서 독립 라인으로 만들기
        text = re.sub(r'([^(\n]+)\s*(\(출처:[^)]+\))', r'\1\n\2', text)  # 제목과 출처 분리
        
        # ▶ 섹션 구분자 앞뒤로 줄바꿈
        text = re.sub(r'([^\n])\s*(▶)', r'\1\n\n\2', text)  # ▶ 앞에 2줄 줄바꿈 (섹션 간 구분)
        text = re.sub(r'(▶[^\n:]+:?)\s*\n*', r'\1\n', text)  # ▶ 제목 뒤에 정확히 1줄만
...
독자가 끝까지 흥미롭게 읽을 수 있는 8000자 이상 한국어 블로그를 작성하세요. 제목은 반드시 매력적인 한국어로!"""
...
**🚨🚨🚨 최우선 필수 규칙 - 제목 이모지와 출처 표기 🚨🚨🚨**
✅ **제목 앞에 내용과 관련된 적절한 이모지 1개 추가**
   - 경제/금융: 📈 (상승), 📉 (하락), 💰 (돈/투자), 💵 (달러), 💴 (엔화), 💶 (유로)
   - 기업/비즈니스: 🏢 (기업), 🏭 (산업), 🛍️ (리테일), 🏦 (은행)
   - 기술/혁신: 🚀 (성장/혁신), 💡 (아이디어), 🔬 (연구), 🤖 (AI), 💻 (IT)
   - 위기/경고: 🚨 (긴급/경고), ⚠️ (주의), 🔥 (핫이슈), 💥 (충격)
   - 국가별: 🇺🇸 (미국), 🇨🇳 (중국), 🇯🇵 (일본), 🇰🇷 (한국), 🇪🇺 (EU)
   - 기타: 📊 (데이터/통계), 🎯 (목표), ⚡ (속보), 🌍 (글로벌)
...
280자 내외로 핵심을 압축하여 작성하세요. 각 문장은 불렛 포인트(•)로 시작하고 줄바꿈으로 구분하며, 간결한 명사형 종결어미(~임, ~음, ~함)를 사용하세요."""
        ...
        # 🚨 중요: 불렛포인트 줄바꿈 문제 완전 해결
        # 1. 연속된 불렛포인트를 줄바꿈으로 분리 (더 강력한 패턴)
        cleaned_response = re.sub(r'(•[^•\n]*?)(•)', r'\1\n\2', cleaned_response)
        
        # 2. 불렛포인트 앞에 공백이나 다른 텍스트가 있는 경우 분리
        cleaned_response = re.sub(r'([^\n])\s*(•)', r'\1\n\2', cleaned_response)
        
        # 3. 출처 뒤에 불렛포인트가 바로 오는 경우 분리
        cleaned_response = re.sub(r'(\(출처:[^)]+\))\s*(•)', r'\1\n\n\2', cleaned_response)
        
        # 4. 제목 뒤에 불렛포인트가 바로 오는 경우 분리
        cleaned_response = re.sub(r'([🚨📈📉📊💰💵💴💶🏢🏭🛍️🏦🚀💡🔬🤖💻⚠️🔥💥🇺🇸🇨🇳🇯🇵🇰🇷🇪🇺🎯⚡🌍📱🏆🎮🛡️📌🔍🌟][^\n]*?)(•)', r'\1\n\n\2', cleaned_response)
        
        # 5. 모든 불렛포인트가 독립된 줄에 있는지 최종 확인
        lines = cleaned_response.split('\n')
        final_lines = []
