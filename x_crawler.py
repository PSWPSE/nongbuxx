...
"""
            ...
            # 각 포스트를 지정된 포맷으로 변환
            for idx, post in enumerate(posts_to_summarize):
                # 넘버링 선택 (10개 이상은 숫자로 표시)
                number_emoji = number_emojis[idx] if idx < 10 else f"({idx+1})"
...
"""
                    ...
                    if post.get('original_author') and post.get('original_content'):
                        ...
                        prompt += f"""   💬 참조: @{post['original_author']}
